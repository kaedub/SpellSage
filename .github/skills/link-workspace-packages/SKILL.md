...
description: 'Link workspace packages in monorepos (npm, yarn, pnpm, bun). USE WHEN: (1) you just created or generated new packages and need to wire up their dependencies, (2) user imports from a sibling package and needs to add it as a dependency, (3) you get resolution errors for workspace packages (@spellsage/*) like "cannot find module", "failed to resolve import", "TS2307", or "cannot resolve". DO NOT patch around with tsconfig paths or manual package.json edits - use the package manager''s workspace commands to fix actual linking.'
---
...
pnpm add @spellsage/ui --workspace

# Or with --filter from anywhere
pnpm add @spellsage/ui --filter @spellsage/app --workspace
```

Result in `package.json`:

```json
{ "dependencies": { "@spellsage/ui": "workspace:*" } }
```
...
```bash
yarn workspace @spellsage/app add @spellsage/ui
```
...
```bash
npm install @spellsage/ui --workspace @spellsage/app
```

Result in `package.json`:

```json
{ "dependencies": { "@spellsage/ui": "*" } }
```
...
```bash
cd packages/app && bun add @spellsage/ui
```
...
```bash
npm install @spellsage/data-access @spellsage/ui --workspace @spellsage/dashboard
```
...
- Symlinks appear in `<consumer>/node_modules/@spellsage/<package>`
